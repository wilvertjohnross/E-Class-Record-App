...
function isNameHeader(t){return t==='name'||t.includes('name of learner')||t.includes('learner name')||t.includes('learners name')||t.includes('full name')||(t.startsWith('name ')&&t.includes('last name')&&t.includes('first name'));}
function isLastNameHeader(t){return (!t.startsWith('name ')&&t.includes('last name'))||t==='surname'||t.includes('family name');}
function isFirstNameHeader(t){return (!t.startsWith('name ')&&t.includes('first name'))||t.includes('given name');}
function isMiddleNameHeader(t){return !t.startsWith('name ')&&t.includes('middle name');}
function isExtensionHeader(t){return t.includes('name extension')||t==='extension'||t==='suffix';}
function isSexHeader(t){return t==='sex'||t==='m f'||t==='gender'||t.includes('sex gender')||t.includes('sex m f');}
...
function sf1HeaderSearchRows(sheet,headerRow,maxC){
  const rows=[];for(let r=Math.max(0,headerRow);r<=Math.min(sheet.maxRow,headerRow+2);r++)rows.push(r);return rows;
}
function findSf1HeaderCol(sheet,headerRow,predicate,maxC=140){
  for(const r of sf1HeaderSearchRows(sheet,headerRow,maxC))for(let c=0;c<=Math.min(sheet.maxCol,maxC);c++){const t=semantic(sheet.get(r,c));if(t&&predicate(t,{r,c}))return c;}return-1;
}
function detectSf1ExtendedColumns(sheet,header){
  const maxC=Math.min(Math.max(sheet.maxCol,0),140),r=header.row;
  const col=(pred)=>findSf1HeaderCol(sheet,r,pred,maxC);
  const guardianGroup=col(t=>t==='guardian'||t.startsWith('guardian if not parent'));
  const contact=col(t=>t.includes('contact number')&&(t.includes('parent')||t.includes('guardian')));
  let guardianName=-1;
  if(guardianGroup>=0){for(let rr=r;rr<=Math.min(sheet.maxRow,r+2)&&guardianName<0;rr++)for(let c=guardianGroup;c<=(contact>=0?contact-1:Math.min(maxC,guardianGroup+8));c++){const t=semantic(sheet.get(rr,c));if(t==='name'){guardianName=c;break;}}}
  return{
    motherTongue:col(t=>t.includes('mother tongue')),
    ipEthnicGroup:col(t=>t.includes('ethnic group')||t==='ip'),
    religion:col(t=>t==='religion'),
    houseStreet:col(t=>t.includes('house')&&(t.includes('street')||t.includes('sitio')||t.includes('purok'))),
    barangay:col(t=>t==='barangay'),
    municipalityCity:col(t=>t.includes('municipality')&&t.includes('city')),
    province:col(t=>t==='province'),
    fatherName:col(t=>t.includes('fathers name')||t.includes('father name')),
    motherName:col(t=>t.includes('mothers maiden name')||t.includes('mother maiden name')),
    guardianName,
    guardianRelationship:col(t=>t==='relationship'||t.includes('relationship to learner')),
    contactNumber:contact,
    learningModality:col(t=>t.includes('learning modality')),
    remarks:col(t=>t==='remarks'||t.startsWith('remarks '))
  };
}
function sf1ExtendedProfile(sheet,row,cols){
  const v=k=>cols[k]>=0?text(sheet.get(row,cols[k])):'';
  return{
    motherTongue:v('motherTongue'),ipEthnicGroup:v('ipEthnicGroup'),religion:v('religion'),
    houseStreet:v('houseStreet'),barangay:v('barangay'),municipalityCity:v('municipalityCity'),province:v('province'),
    fatherName:v('fatherName'),motherName:v('motherName'),guardianName:v('guardianName'),guardianRelationship:v('guardianRelationship'),
    contactNumber:v('contactNumber'),learningModality:v('learningModality'),remarks:v('remarks')
  };
}

function detectSf1Header(sheet){let best=null;const maxR=Math.min(Math.max(sheet.maxRow,0),70),maxC=Math.min(Math.max(sheet.maxCol,0),140);for(let r=0;r<=maxR;r++){const nextIsLearner=Array.from({length:maxC+1},(_,i)=>text(sheet.get(r+1,i))).some(v=>/^\d{12}$/.test(v));for(const single of [true,false]){const cols={lrn:-1,name:-1,last:-1,first:-1,middle:-1,ext:-1,sex:-1,birth:-1,age:-1};for(let c=0;c<=maxC;c++){const t=single||nextIsLearner?semantic(sheet.get(r,c)):labelWindow(sheet,r,c);if(!t)continue;if(cols.lrn<0&&isLrnHeader(t))cols.lrn=c;if(cols.name<0&&isNameHeader(t)&&!isLastNameHeader(t)&&!isFirstNameHeader(t)&&!isMiddleNameHeader(t))cols.name=c;if(cols.last<0&&isLastNameHeader(t))cols.last=c;if(cols.first<0&&isFirstNameHeader(t))cols.first=c;if(cols.middle<0&&isMiddleNameHeader(t))cols.middle=c;if(cols.ext<0&&isExtensionHeader(t))cols.ext=c;if(cols.sex<0&&isSexHeader(t))cols.sex=c;if(cols.birth<0&&isBirthHeader(t))cols.birth=c;if(cols.age<0&&isAgeHeader(t))cols.age=c;}
...
function findMetaValue(sheet,synonyms,maxRows=35,maxCols=140){const wanted=synonyms.map(semantic);for(let r=0;r<=Math.min(sheet.maxRow,maxRows);r++)for(let c=0;c<=Math.min(sheet.maxCol,maxCols);c++){const raw=text(sheet.get(r,c)),t=semantic(raw);if(!t)continue;let matched=false;for(const w of wanted){if(t===w||t.startsWith(`${w} `)){matched=true;const colon=raw.indexOf(':');if(colon>=0&&text(raw.slice(colon+1)))return text(raw.slice(colon+1));break;}}if(!matched)continue;for(let d=1;d<=12&&c+d<=Math.min(sheet.maxCol,maxCols);d++){const v=text(sheet.get(r,c+d));if(v&&!wanted.includes(semantic(v)))return v;}for(let dr=1;dr<=2&&r+dr<=sheet.maxRow;dr++){for(let dc=0;dc<=3&&c+dc<=sheet.maxCol;dc++){const v=text(sheet.get(r+dr,c+dc));if(v)return v;}}}return'';}
function findBelowLabelValue(sheet,synonyms,maxRows=120,maxCols=140){const wanted=synonyms.map(semantic);for(let r=0;r<=Math.min(sheet.maxRow,maxRows);r++)for(let c=0;c<=Math.min(sheet.maxCol,maxCols);c++){const t=semantic(sheet.get(r,c));if(!t||!wanted.some(w=>t===w||t.startsWith(`${w} `)))continue;for(let dr=1;dr<=4&&r+dr<=sheet.maxRow;dr++){for(let dc=0;dc<=2&&c+dc<=sheet.maxCol;dc++){const v=text(sheet.get(r+dr,c+dc)),sv=semantic(v);if(!v||wanted.includes(sv)||sv.includes('signature of')||sv==='prepared by'||sv.startsWith('certified correct'))continue;return v;}}}return'';}
function sf1Meta(sheet){let region='';for(let r=0;r<=Math.min(sheet.maxRow,15)&&!region;r++)for(let c=0;c<=Math.min(sheet.maxCol,80);c++){const v=text(sheet.get(r,c));if(/^Region\s+[IVX0-9-]+$/i.test(v)){region=v;break;}}if(!region)region=findMetaValue(sheet,['Region']);return{schoolId:findMetaValue(sheet,['School ID','School Id No','School Number']),region,division:findMetaValue(sheet,['Division','Schools Division']),schoolName:findMetaValue(sheet,['School Name','Name of School']),schoolYear:findMetaValue(sheet,['School Year','SY','S Y']),gradeLevel:findMetaValue(sheet,['Grade Level','Grade','Year Level']),section:findMetaValue(sheet,['Section']),adviser:findBelowLabelValue(sheet,['Prepared by']),schoolHead:findBelowLabelValue(sheet,['Certified Correct'])};}
function parseSf1Sheet(sheet){const header=detectSf1Header(sheet);if(!header)return null;const {cols}=header,extendedCols=detectSf1ExtendedColumns(sheet,header),warnings=[],learners=[],seen=new Set();let currentSex='',blankRun=0,sexEvidence=cols.sex>=0;const maxC=Math.min(Math.max(sheet.maxCol,0),140);for(let r=header.row+1;r<=Math.min(sheet.maxRow,header.row+500);r++){const rowHeaderText=semantic(sheet.rowText(r,0,maxC));if((rowHeaderText.includes('lrn')||rowHeaderText.includes('learner reference'))&&(rowHeaderText.includes('name')||rowHeaderText.includes('surname')))continue;const marker=rowSexMarker(sheet,r,maxC);if(marker){currentSex=marker;sexEvidence=true;continue;}const name=composeName(sheet,r,cols),rawLrn=sheet.get(r,cols.lrn),lrn=normalizeLrn(rawLrn),rowSem=semantic(sheet.rowText(r,0,maxC));if(rowSem.includes('total male')||rowSem.includes('total female')||rowSem.includes('combined')||rowSem.includes('list and code of indicators')||rowSem.includes('generated thru lis')||rowSem.includes('generated on')||rowSem.includes('prepared by')||rowSem.includes('certified correct'))continue;if(!name&&!lrn){if(learners.length&&++blankRun>50)break;continue;}blankRun=0;if(isExcelError(name)){warnings.push(`Row ${r+1} contains an unresolved Excel value instead of a learner name and was skipped.`);continue;}if(!name){warnings.push(`Row ${r+1} has an LRN but no learner name and was skipped.`);continue;}if(!/^\d{12}$/.test(lrn)){warnings.push(`Row ${r+1} (${name}) does not contain a valid 12-digit LRN and was skipped.`);continue;}if(seen.has(lrn)){warnings.push(`Duplicate LRN ${lrn} was found; the later duplicate at row ${r+1} was skipped.`);continue;}const sex=cols.sex>=0?normalizeSex(sheet.get(r,cols.sex)):currentSex;if(!sex){warnings.push(`Row ${r+1} (${name}) has no recognizable sex value or Male/Female section marker and was skipped.`);continue;}const age=cols.age>=0?normalizeAge(sheet.get(r,cols.age)):'',birthDate=cols.birth>=0?excelSerialToIso(sheet.get(r,cols.birth)):'';seen.add(lrn);learners.push({lrn,name,sex,age,birthDate,sf1:sf1ExtendedProfile(sheet,r,extendedCols),row:r+1});}
